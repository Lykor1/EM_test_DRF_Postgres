...
'''class AuthMiddleware:
    ...
    def __init__(self, get_response):
        ...
        self.public_routes = ('register', 'login', 'index')
    ...
    def __call__(self, request):
        ...
        print("Resolved URL:", resolved.url_name)  # Отладка
        print("Headers:", request.headers)  # Отладка
        if resolved.url_name in self.public_routes:
            return self.get_response(request)
        auth_header = request.headers.get('Authorization', '')
        print("Auth header:", auth_header)  # Отладка
...
class AuthMiddleware:
    ...
    def __call__(self, request):
        ...
        if resolved.url_name in self.private_routes:
            resource_name = resolved.url_name
            rules = AccessRule.objects.filter(role=request.user.role, resource__name=resource_name)
            if not rules.exists():
                return JsonResponse({'error': 'Forbidden'}, status=403)
            permission_field = self.method_permissions.get(request.method)
            if permission_field and not rules.filter(**{permission_field: True}).exists():
